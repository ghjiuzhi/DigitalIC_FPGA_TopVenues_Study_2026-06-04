# paper_06 - Bank on Compute-Near-Memory: Design Space Exploration of Processing-Near-Bank Architectures

## 一句话定位
这篇 TCAD 论文围绕 **Bank on Compute-Near-Memory: Design Space Exploration of Processing-Near-Bank Architectures** 展开，核心是把一个明确的硬件/EDA/安全问题压缩成可实现、可度量、可对比的设计贡献。

## 为什么属于数字 IC / FPGA 方向
它围绕 SRAM/MRAM/RRAM/CAM/CIM/PIM、memristor 或近存计算，属于数字存储、存算融合或新型计算电路。 本篇在 metadata 中标注的相关性是：digital IC / architecture, memory / CIM。

## 摘要写法
摘要适合按“问题对象 -> proposed method/architecture -> implementation/evaluation -> quantitative comparison”组织。对这篇题目来说，第一句应点出 digital IC / architecture, memory / CIM 的瓶颈，中间一句说明核心机制，最后用硬件指标或安全/精度指标收束 claim。
