# paper_01 - CORDIC-Based Computation of Arcsine and Arccosine Functions on FPGA

## 一句话定位
这篇 TCAS-II 论文围绕 **CORDIC-Based Computation of Arcsine and Arccosine Functions on FPGA** 展开，核心是把一个明确的硬件/EDA/安全问题压缩成可实现、可度量、可对比的设计贡献。

## 为什么属于数字 IC / FPGA 方向
它直接围绕 FPGA 上的结构映射、资源占用、时延、吞吐或可靠性展开，是数字硬件实现论文的典型样本。 本篇在 metadata 中标注的相关性是：FPGA arithmetic architecture。

## 摘要写法
摘要适合按“问题对象 -> proposed method/architecture -> implementation/evaluation -> quantitative comparison”组织。对这篇题目来说，第一句应点出 FPGA arithmetic architecture 的瓶颈，中间一句说明核心机制，最后用硬件指标或安全/精度指标收束 claim。
